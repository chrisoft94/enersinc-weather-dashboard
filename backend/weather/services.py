    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.warning(f"Intento {attempt + 1} fallido para {city_name}. Reintentando en {retry_delay}s... (Error: {e})")
                time.sleep(retry_delay)
            else:
                logger.error(f"Error definitivo consultando OpenWeather para {city_name} después de {max_retries} intentos: {e}")
